...
/**
 * Asynchronous, dialect-agnostic access to a SQL database. Every method returns a {@link CompletableFuture} executed
 * off the main server thread; callers resume on the main thread (when touching the Bukkit API) using an appropriate
 * executor, e.g. {@code SchedulerService#mainThreadExecutor()}.
 *
 * <p>
 * Implementations are backed by a connection pool. This interface is intentionally free of any Bukkit dependency so it
...
public interface Database extends AutoCloseable {
    ...
    /**
     * Borrows a raw connection from the pool. The caller is responsible for closing it (preferably with
     * try-with-resources). Prefer the higher-level query methods where possible.
     */
    ...
    /**
     * Executes arbitrary work against a borrowed connection (e.g. a multi-statement transaction). The connection is
     * closed automatically after the work completes.
     */
