...
/**
 * A minimal, type-keyed service locator. Modules register the services they own and resolve services owned by other
 * modules through this registry, avoiding direct references between subsystems.
 *
 * <p>
 * This is intentionally simple (no scopes, no proxies): the goal is decoupling and testability, not a full
...
public final class ServiceRegistry {
     ...
     *
     * @param type
     *            the interface or class other components will look the service up by
     * @param service
     *            the implementation instance
     * @param <T>
     *            the service contract type
     * @throws IllegalStateException
     *             if a service is already registered for {@code type}
     */
     ...
     *
     * @throws IllegalStateException
     *             if no service is registered for {@code type}
     */
    public <T extends Service> T get(Class<T> type) {
        return find(type)
                .orElseThrow(() -> new IllegalStateException("No service registered for type " + type.getName()));
    }
