...
/**
 * Reference plugin wired entirely from framework building blocks. It exists to document the
 * intended usage end-to-end; a real plugin replaces this class (and the {@code example} package)
 * with its own {@link CorePlugin} subclass and modules.
 *
 * <p>The module order is the startup order: foundation services first, then the NMS adapter (so any
 * later module can resolve it), then integrations, the menu subsystem, and finally the commands that
 * depend on all of the above.</p>
 */
public class ExamplePlugin extends CorePlugin {
    ...
    protected String[] banner() {
        return new String[]{
                "",
                "  DzusillCore " + getDescription().getVersion(),
