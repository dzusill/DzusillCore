...
/**
 * Wires up optional third-party integrations. Hooks are registered lazily by plugin name, so a hook class (and the
 * soft-dependency API types it imports) is only loaded when that plugin is actually installed. This keeps the template
 * safe whether the server runs none of these plugins or all of them.
 */
