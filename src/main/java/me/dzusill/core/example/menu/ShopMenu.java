...
/**
 * Example menu showing the declarative GUI style that mirrors the command system: title, size and open-permission come
 * from {@link MenuMeta}, and slots are declared with the fluent, permission aware {@link #button(int)} API. The
 * bordered template fills the edges; only the interactive buttons are declared here. The "restock" button carries a
 * permission, so it is automatically hidden from players who lack {@code core.shop.admin}.
 */
...
public final class ShopMenu extends Menu {
    ...
    protected void decorate() {
        button(13)
                .icon(new ItemBuilder(Material.DIAMOND).name("<aqua>Buy a diamond").lore("<gray>Click to purchase")
                        .glow().build())
                .onClick(event -> messages().sendComponent(context.player(),
                        ColorUtils.parse("<green>Purchased a diamond!")))
                .add();

        button(15).icon(new ItemBuilder(Material.CHEST).name("<gold>Restock").lore("<gray>Admins only").build())
                .permission("core.shop.admin")
                .onClick(
                        event -> messages().sendComponent(context.player(), ColorUtils.parse("<green>Shop restocked!")))
                .add();
    }
