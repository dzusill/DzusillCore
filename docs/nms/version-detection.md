...
`VersionDetector.detect()` returns an immutable `MinecraftVersion` describing the running server. It must be robust across the whole 1.16.5–1.21.x range, which crosses two breakpoints that catch out naive detection code:

- **1.17** — NMS moved from `net.minecraft.server.v1_16_R3.*` to `net.minecraft.*`.
- **1.20.5** — Paper stopped relocating CraftBukkit into a `vX_Y_RZ` package and switched to Mojang mappings. From here the classic "read the `v1_20_R4` suffix off the server package" trick **returns nothing**.
...
`VersionDetector.detect()` runs under MockBukkit (it emulates a modern server), so detection and adapter selection are unit-tested. Exact NMS calls cannot run under MockBukkit — verify those on real servers (see [Adapters](adapters.md#manual-verification-matrix)).
